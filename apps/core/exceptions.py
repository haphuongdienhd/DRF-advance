...
class GenericException(APIException):
    ...
    def __init__(self, message=None, status_code=400, error_detail=None, language_code=None):
        if not message:
    ...
    def serialize(self, language_code=None):
        language_code = language_code if not self.language_code else self.language_code
        data = {
...
class MissingSecretQuestionException(GenericException):
    code = 1012

    def __init__(self, message=None):
        if not message:
            message = "Missing secret question."
        super().__init__(message)


class InvalidSecretQuestionAnswerException(GenericException):
    code = 1013
